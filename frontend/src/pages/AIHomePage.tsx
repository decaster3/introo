...
export function AIHomePage() {
  ...
  return (
    <div className="u-root">
      ...
      <nav className="u-mobile-tabs">
        <button
          className={`u-mobile-tab ${!inlinePanel ? 'u-mobile-tab--active' : ''}`}
          onClick={() => { setInlinePanel(null); setSidebarOpen(false); }}
        >
          <span className="u-mobile-tab-icon">
        ...
        <button
          className={`u-mobile-tab ${inlinePanel?.type === 'network-manage' ? 'u-mobile-tab--active' : ''}`}
          onClick={() => setInlinePanel(inlinePanel?.type === 'network-manage' ? null : { type: 'network-manage' })}
        >
          <span className="u-mobile-tab-icon">
        ...
        <button
          className={`u-mobile-tab ${inlinePanel?.type === 'notifications' ? 'u-mobile-tab--active' : ''}`}
          onClick={() => {
            if (inlinePanel?.type === 'notifications') {
              setInlinePanel(null);
            } else {
              setInlinePanel({ type: 'notifications' });
              notificationsApi.markAllRead().then(() => setNotificationCount(0));
            }
          }}
        >
          <span className="u-mobile-tab-icon">
          ...
          Alerts
          {notificationCount > 0 && <span className="u-mobile-tab-badge">{notificationCount}</span>}
        </button>
        <button
          className={`u-mobile-tab ${inlinePanel?.type === 'settings' ? 'u-mobile-tab--active' : ''}`}
          onClick={() => setInlinePanel(inlinePanel?.type === 'settings' ? null : { type: 'settings' })}
        >
          <span className="u-mobile-tab-icon">
